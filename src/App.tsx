...
import { fmtBoth, fmtDateTime, fmtKrw, fmtSigned, krwToJpy, fmtJpy } from './lib/money'
...
export default function App() {
  ...
  return (
    <div
      className="mx-auto flex h-dvh max-w-md flex-col"
      data-casino={session?.casino ?? settings.casino ?? 'PARADISE'}
    >
      <Header />
  ...
  return (
    <div
      className={`fixed inset-0 z-50 flex flex-col items-center justify-center gap-6 p-8 ${
        isSl ? 'bg-[#31090c]/95' : 'bg-base-900/95'
      }`}
    >
      <div className="text-5xl">{isSl ? '🛑' : '🎯'}</div>
      <h1 className={`text-2xl font-bold ${isSl ? 'text-lose' : 'text-gold-300'}`}>
...
function SessionSummary() {
  ...
  return (
    <div className="space-y-3 pb-2">
      <div className="rounded-xl bg-base-950 p-4 text-center">
        <div className="text-xs text-ink-3">最終収支</div>
        <div className={`num text-3xl font-bold ${pl > 0 ? 'text-win' : pl < 0 ? 'text-lose' : ''}`}>
          {fmtSigned(pl)}
        </div>
      ...
      <div className="grid grid-cols-3 gap-2 text-center">
        <div className="rounded-lg bg-base-950 p-3">
      ...
      {s.startInput?.currency === 'JPY' && s.startInput.rate != null && (
        <p className="num text-center text-[10px] text-ink-3">
          開始資金 ¥{s.startInput.amount.toLocaleString()} を ¥1=₩{s.startInput.rate.toFixed(2)}
          {s.startInput.rateTs != null ? `(${fmtDateTime(s.startInput.rateTs)} 時点)` : ''}で
          {fmtKrw(s.startKrw)} に換算して開始
        </p>
      )}
    </div>
  )
