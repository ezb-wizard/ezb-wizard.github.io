  ...
  return (
    <div className="space-y-2">
      ...
      {adding ? (
        ...
        <button
          className="h-12 w-full rounded-lg border border-dashed border-felt-700 text-sm font-bold text-ink-2"
...
function SideBetRow({
  ...
  return (
    <div className={`rounded-lg border p-3 ${def.enabled ? 'border-gold-600 bg-felt-900' : 'border-felt-800 bg-felt-950 opacity-70'}`}>
      <div className="flex items-center justify-between">
        <button className="flex min-h-12 flex-1 items-center gap-2 text-left" onClick={onToggle}>
          <span
            className={`flex h-6 w-11 items-center rounded-full p-0.5 transition-colors ${def.enabled ? 'bg-gold-500' : 'bg-felt-700'}`}
          >
            <span
          ...
          <span
            className={`rounded px-1.5 py-0.5 text-[10px] font-bold text-felt-950 ${
        ...
        {def.rules.map((r, i) => (
          <div key={i} className="flex items-center gap-2 text-xs text-ink-2">
            <span className="flex-1">
              {r.totals.length ? `合計${r.totals.join('・')}` : '合計任意'} / {CARD_LABEL[r.cards]}
            </span>
            <NumInput
              value={r.payout}
      ...
      {stats && (
        <div className="num mt-1 text-right text-[11px]">
          ...
          <span className={stats.edge > 0.1 ? 'font-bold text-banker' : 'text-ink-2'}>
...
function CustomForm({ onAdd, onCancel }: { onAdd: (d: SideBetDef) => void; onCancel: () => void }) {
  ...
  return (
    <div className="space-y-3 rounded-lg border border-gold-600 bg-felt-900 p-3">
      <Field label="名称">
        <input
          type="text"
            ...
            <button
              ...
              className={`h-12 w-[calc(20%-6px)] rounded-lg border text-sm font-bold ${
                totals.includes(n)
                  ...
                  : 'border-felt-700 bg-felt-950 text-ink-2'
              }`}
              onClick={() =>
      ...
      <div className="flex gap-2">
        <button className="h-12 flex-1 rounded-lg border border-felt-700 text-sm font-bold text-ink-2" onClick={onCancel}>
          キャンセル
        </button>
        <button
          className="h-12 flex-1 rounded-lg bg-gold-500 text-sm font-bold text-felt-950 disabled:opacity-40"
          disabled={!valid}
          onClick={() =>
            onAdd({
              id: `custom_${Date.now()}`,
